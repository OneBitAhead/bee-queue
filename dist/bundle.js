...
`});var F=c((Ce,N)=>{"use strict";var he=require("node:crypto"),O={addDelayedJob:D(),addJob:A(),checkStalledJobs:R(),raiseDelayedJobs:P(),removeJob:Y()},k={},p={};function ce(n,e){k[n]=e;let t=he.createHash("sha1");t.update(e),p[n]=t.digest("hex")}function de(){for(var n in O)ce(n,O[n])}async function ue(n,e){var t=await new Promise((i,s)=>{n.script("exists",p[e],i)});t===null&&await new Promise((i,s)=>{n.script("load",k[e],i)})}async function fe(n){return de(),Promise.all(Object.keys(p).map(e=>ue(n,e)))}N.exports={scripts:k,shas:p,buildCache:fe}});var G=c((je,z)=>{"use strict";var g=new Map;g.set("immediate",()=>0);g.set("fixed",n=>n.options.backoff.delay);g.set("exponential",n=>{let e=n.options.backoff,t=e.delay;return e.delay*=2,t});z.exports=g});var V=c((Te,M)=>{"use strict";var me=require("node:events").EventEmitter,v=class extends me{constructor(e){if(super(),!Number.isSafeInteger(e)||e<=0)throw new Error("maximum delay must be a positive integer");this._maxDelay=e,this._nextTime=null,this._timer=null,this._stopped=!1,this._boundTrigger=this._trigger.bind(this)}schedule(e){if(this._stopped)return;let t=Date.now();if(e<0||e==null||isNaN(e))e=t+this._maxDelay;else{if(e<=t)return this._schedule(t+this._maxDelay),this.emit("trigger");e=Math.min(e,t+this._maxDelay)}(!this._timer||e<this._nextTime)&&this._schedule(e)}stop(){this._stop(),this._stopped=!0}_stop(){this._timer&&(clearTimeout(this._timer),this._nextTime=null,this._timer=null)}_schedule(e){let t=e-Date.now();this._stop(),this._nextTime=e,this._timer=setTimeout(this._boundTrigger,t)}_trigger(){let e=Date.now(),t=this._nextTime-e;if(t>0){this._timer=setTimeout(this._boundTrigger,t);return}this._schedule(e+this._maxDelay),this.emit("trigger")}};M.exports=v});var B=c((Je,U)=>{"use strict";var b=q(),ye=require("node:events").EventEmitter,_=T(),y=x(),W=F(),l=w(),be=G(),pe=V(),S=class extends ye{constructor(e,t={}){super(),this.name=e,this.paused=!1,this.jobs=new Map,this.activeJobs=new Set,this.checkTimer=null,this.backoffStrategies=new Map(be),this._closed=null,this._isClosed=!1,this._emitError=i=>void this.emit("error",i),this._emitErrorAfterTick=i=>void process.nextTick(()=>this.emit("error",i)),this.client=null,this.bclient=null,this.eclient=null,this.settings={redis:t.redis||{},quitCommandClient:t.quitCommandClient,keyPrefix:(t.prefix||y.prefix)+":"+this.name+":",autoConnect:l.bool(t.autoConnect,!0)},this._isReady=!1,this._ready=!1;for(let i in y){let s=y[i],r=t[i],a=typeof s;a==="boolean"?this.settings[i]=typeof r=="boolean"?r:s:a==="number"&&(this.settings[i]=Number.isSafeInteger(r)?r:s)}this.settings.redis.socket&&(this.settings.redis=Object.assign({},this.settings.redis,{path:this.settings.redis.socket})),typeof this.settings.quitCommandClient!="boolean"&&(this.settings.quitCommandClient=!b.isClient(this.settings.redis)),this._delayedTimer=this.settings.activateDelayedJobs?new pe(this.settings.nearTermWindow):null,this._delayedTimer&&this._delayedTimer.on("trigger",this._activateDelayed.bind(this)),this.settings.autoConnect&&this.connect()}makeClient(e,t){return b.createClient(this.settings.redis,t).then(i=>(i.on("error",this._emitError),this[e]=i))}connect(){return new Promise((e,t)=>{try{if(this._isReady)return e(this._isReady);let s=(()=>this.settings.getEvents||this.settings.activateDelayedJobs?this.makeClient("eclient",!0).then(()=>{this.eclient.on("message",this._onMessage.bind(this));let r=[];return this.settings.getEvents&&r.push(this.toKey("events")),this.settings.activateDelayedJobs&&r.push(this.toKey("earlierDelayed")),Promise.all(r.map(a=>l.callAsync(h=>this.eclient.subscribe(a,h))))}):null)();this._ready=Promise.all([this.makeClient("client",!1),this.settings.isWorker?this.makeClient("bclient",!0):null,s]).then(()=>{if(this.settings.ensureScripts)return W.buildCache(this.client)}).then(()=>(this._isReady=!0,setImmediate(()=>this.emit("ready")),e(this._isReady),this))}catch(i){t(i)}})}_onMessage(e,t){if(e===this.toKey("earlierDelayed")){this._delayedTimer.schedule(parseInt(t,10));return}t=JSON.parse(t),(t.event==="failed"||t.event==="retrying")&&(t.data=new Error(t.data)),this.emit("job "+t.event,t.id,t.data);let i=this.jobs.get(t.id);i&&(t.event==="progress"?i.progress=t.data:t.event==="retrying"&&(i.options.retries-=1),i.emit(t.event,t.data),(t.event==="succeeded"||t.event==="failed")&&this.jobs.delete(t.id))}isRunning(){return!this.paused}ready(e){return e&&l.asCallback(this._ready.then(()=>{}),e),this._ready}_commandable(e){return(e?this.paused:this._isClosed)?Promise.reject(new Error("closed")):this._isReady?Promise.resolve(e?this.bclient:this.client):this._ready.then(()=>this._commandable(e))}close(e,t){if(typeof e=="function"?(t=e,e=y["#close"].timeout):(!Number.isSafeInteger(e)||e<=0)&&(e=y["#close"].timeout),this.paused)return t&&l.asCallback(this._closed,t),this._closed;this.paused=!0,this.checkTimer&&(clearTimeout(this.checkTimer),this.checkTimer=null),this._delayedTimer&&this._delayedTimer.stop();let i=()=>l.finallyRejectsWithInitial(this._ready,()=>(this.settings.isWorker&&b.disconnect(this.bclient),Promise.all(Array.from(this.activeJobs,a=>a.catch(()=>{}))).then(()=>{}))),s=()=>{this._isClosed=!0;let a=[];return this.client&&(this.settings.quitCommandClient?a.push(this.client):this.client.removeListener("error",this._emitError)),this.eclient&&a.push(this.eclient),Promise.all(a.map(h=>l.callAsync(o=>h.quit(o))))},r=l.finallyRejectsWithInitial(l.withTimeout(i(),e),()=>s());return this._closed=r,t&&l.asCallback(r,t),r}destroy(e){let t=this._commandable().then(i=>{let s=l.deferred(),r=["id","jobs","stallBlock","stalling","waiting","active","succeeded","failed","delayed"].map(a=>this.toKey(a));return r.push(s.defer()),i.del.apply(i,r),s});return e&&l.asCallback(t,e),t}checkHealth(e){let t=this._commandable().then(i=>l.callAsync(s=>i.multi().llen(this.toKey("waiting")).llen(this.toKey("active")).scard(this.toKey("succeeded")).scard(this.toKey("failed")).zcard(this.toKey("delayed")).get(this.toKey("id")).exec(s))).then(i=>({waiting:i[0],active:i[1],succeeded:i[2],failed:i[3],delayed:i[4],newestJob:i[5]?parseInt(i[5],10):0}));return e&&l.asCallback(t,e),t}_scanForJobs(e,t,i,s,r){let a=Math.min(i,this.settings.redisScanCount);this.client.sscan(e,t,"COUNT",a,(h,o)=>{if(h)return r(h);let d=o[0],H=o[1];for(let L of H){if(s.size===i)break;s.add(L)}if(d==="0"||s.size>=i)return r(null,s);this._scanForJobs(e,d,i,s,r)})}_addJobsByIds(e,t){return this._commandable().then(i=>{let s=l.deferred(),r=[this.toKey("jobs")].concat(t,s.defer());return i.hmget.apply(i,r),s}).then(i=>{let s=t.length;for(let r=0;r<s;++r){let a=i[r];a&&e.push(_.fromData(this,t[r],a))}return e})}getJobs(e,t,i){typeof t=="function"&&(i=t,t=null),t=Object.assign({size:100,start:0,end:100},t);let s=this._commandable().then(r=>{let a=l.deferred(),h=a.defer(),o=this.toKey(e);switch(e){case"failed":case"succeeded":this._scanForJobs(o,"0",t.size,new Set,h);break;case"waiting":case"active":r.lrange(o,t.start,t.end,h);break;case"delayed":r.zrange(o,t.start,t.end,h);break;default:throw new Error("Improper queue type")}return a}).then(r=>{let a=[],h=[];for(let o of r){let d=this.jobs.get(o);d?a.push(d):h.push(o)}return h.length?this._addJobsByIds(a,h):a});return i&&l.asCallback(s,i),s}createJob(e){return new _(this,null,e)}getJob(e,t){let i=this._commandable().then(()=>this.jobs.has(e)?this.jobs.get(e):_.fromId(this,e)).then(s=>(s&&this.settings.storeJobs&&this.jobs.set(e,s),s));return t&&l.asCallback(i,t),i}removeJob(e,t){let i=this._evalScript("removeJob",7,this.toKey("succeeded"),this.toKey("failed"),this.toKey("waiting"),this.toKey("active"),this.toKey("stalling"),this.toKey("jobs"),this.toKey("delayed"),e).then(()=>(this.settings.storeJobs&&this.jobs.delete(e),this));return t&&l.asCallback(i,t),i}_waitForJob(){return l.callAsync(e=>this.bclient.brpoplpush(this.toKey("waiting"),this.toKey("active"),0,e)).then(e=>_.fromId(this,e),e=>b.isAbortError(e)&&this.paused?null:(this.emit("error",e),this._redisFailureRetryDelay=this._redisFailureRetryDelay?this._redisFailureRetryDelay*2:this.settings.initialRedisFailureRetryDelay,l.delay(this._redisFailureRetryDelay).then(()=>this._waitForJob())))}_getNextJob(){return this._redisFailureRetryDelay=0,this._commandable(!0).then(()=>this._waitForJob())}_runJob(e){let t=null,i=!1,s=()=>{t=null,!this._isClosed&&this._preventStall(e.id).catch(this._emitErrorAfterTick).finally(()=>{if(i||this._isClosed)return;let o=this.settings.stallInterval/2;t=setTimeout(s,o)})};s();let r=(o,d)=>(i=!0,t&&(clearTimeout(t),t=null),this._finishJob(o,d,e)),a=this.handler(e);if(e.options.timeout){let o=`Job ${e.id} timed out (${e.options.timeout} ms)`;a=l.withTimeout(a,e.options.timeout,o)}let h=a.then(o=>r(null,o),r).finally(()=>{this.activeJobs.delete(h)});return this.activeJobs.add(h),h}_preventStall(e){return l.callAsync(t=>this.client.srem(this.toKey("stalling"),e,t))}_finishJob(e,t,i){if(this._isClosed){let o=e?"failed":"succeeded";throw new Error(`unable to update the status of ${o} job ${i.id}`)}let s=this.client.multi().lrem(this.toKey("active"),0,i.id).srem(this.toKey("stalling"),i.id),r=e?i.computeDelay():-1,a=e?r>=0?"retrying":"failed":"succeeded";if(i.status=a,e){let o=e.stack||e.message||e;i.options.stacktraces.unshift(o)}switch(a){case"failed":this.settings.removeOnFailure?s.hdel(this.toKey("jobs"),i.id):(s.hset(this.toKey("jobs"),i.id,i.toData()),s.sadd(this.toKey("failed"),i.id));break;case"retrying":if(--i.options.retries,s.hset(this.toKey("jobs"),i.id,i.toData()),r===0)s.lpush(this.toKey("waiting"),i.id);else{let o=Date.now()+r;s.zadd(this.toKey("delayed"),o,i.id).publish(this.toKey("earlierDelayed"),o)}break;case"succeeded":this.settings.removeOnSuccess?s.hdel(this.toKey("jobs"),i.id):(s.hset(this.toKey("jobs"),i.id,i.toData()),s.sadd(this.toKey("succeeded"),i.id));break}this.settings.sendEvents&&s.publish(this.toKey("events"),JSON.stringify({id:i.id,event:a,data:e?e.message:t}));let h=e||t;return l.callAsync(o=>s.exec(o)).then(()=>[a,h])}process(e,t){if(!this.settings.isWorker)throw new Error("Cannot call Queue#process on a non-worker");if(this.handler)throw new Error("Cannot call Queue#process twice");if(this.paused)throw new Error("closed");typeof e=="function"&&(t=e,e=y["#process"].concurrency);let i=!0;this.handler=l.wrapAsync(t,i),this.running=0,this.queued=1,this.concurrency=e;let s=()=>{if(this.paused){this.queued-=1;return}this._getNextJob().then(r=>{if(this.paused){this.queued-=1;return}if(this.running+=1,this.queued-=1,this.running+this.queued<this.concurrency&&(this.queued+=1,setImmediate(s)),!!r)return this._runJob(r).then(a=>{if(this.running-=1,this.queued+=1,a){let h=a[0],o=a[1];this.emit(h,r,o);let d=h==="retrying"?"failed":h==="failed"?"failed:fatal":null;d&&this.emit(d,r,o)}},this._emitErrorAfterTick)}).catch(this._emitErrorAfterTick).finally(()=>setImmediate(s))};return this._doStalledJobCheck().then(s).catch(this._emitErrorAfterTick),this._activateDelayed(),this}_doStalledJobCheck(){return this._evalScript("checkStalledJobs",4,this.toKey("stallBlock"),this.toKey("stalling"),this.toKey("waiting"),this.toKey("active"),this.settings.stallInterval).then(e=>{for(let t of e)this.emit("stalled",t);return e.length})}_safeCheckStalledJobs(e,t){let i=this._checkStalledJobs(e,t);t||i.catch(this._emitErrorAfterTick)}_scheduleStalledCheck(e,t){this.checkTimer||this.paused||(this.checkTimer=setTimeout(()=>{this.checkTimer=null,this._safeCheckStalledJobs(e,t)},e))}_checkStalledJobs(e,t){let i=this._doStalledJobCheck();return t&&l.asCallback(i,t),e&&!this.checkTimer?i.finally(()=>{try{this._scheduleStalledCheck(e,t)}catch(s){this._emitErrorAfterTick(s)}}):i}checkStalledJobs(e,t){return typeof e=="function"?(t=e,e=null):Number.isSafeInteger(e)||(e=null),this._checkStalledJobs(e,t)}saveAll(e){return this._commandable().then(t=>{let i=t.batch(),s=new Map;for(let r of e)try{r._save(a=>this._evalScriptOn(i,a)).catch(a=>void s.set(r,a))}catch(a){s.set(r,a)}return l.callAsync(r=>i.exec(r)).then(()=>s)})}_activateDelayed(){!this.settings.activateDelayedJobs||this._evalScript("raiseDelayedJobs",2,this.toKey("delayed"),this.toKey("waiting"),Date.now(),this.settings.delayedDebounce).then(e=>{let t=e[0],i=e[1];t&&this.emit("raised jobs",t),this._delayedTimer.schedule(parseInt(i,10))},e=>{if(b.isAbortError(e))return this.paused?void 0:this._activateDelayed();this._emitErrorAfterTick(e)})}toKey(e){return this.settings.keyPrefix+e}_evalScriptOn(e,t){return t[0]=W.shas[t[0]],l.callAsync(i=>{t.push(i),e.evalsha.apply(e,t)})}_evalScript(){let e=new Array(arguments.length);for(let t=0;t<arguments.length;++t)e[t]=arguments[t];return this._commandable().then(t=>this._evalScriptOn(t,e))}};U.exports=S});module.exports=B();
